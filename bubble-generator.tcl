...
#	text descriptions.
#
#	Modified by Alexandre Gambier <agambier.dev@gmail.com>
#	Modified version of the origial script (http://www.sqlite.org/docsrc/doc/tip/art/syntax/bubble-generator.tcl?mimetype=text/plain)
#	from the SQLite source files.
#
#	This version accept arguments so it can be used with bgen (https://github.com/agambier/bgen).
#


source [file join [file dirname [info script]] /tmp/bubble-generator-data.tcl]

array set arguments {-show 0 -graph all -dpi 96 -html 0 -gif 0 -preview 0 }
...
set DPI $arguments(-dpi)          ;# dots per inch
...
proc draw_graph {name spec {preview 1} {create_gif 1}} {
	.c delete all
	wm deiconify .
	wm title . $name
	...
	if {$create_gif} {
		...
		if {$preview} {
			if {[catch {exec xdg-open $name.gif &}]} {
...
proc draw_all_graphs {{html 0}} {
	...
	foreach {name graph} $all_graphs {
		...
		draw_graph $name $graph $arguments(-preview) $arguments(-gif)
		set img($name) 1
		set children($name) {}
		set parents($name) {}
	}
...
if {$arguments(-show)} {
	# Show a window with all the graphs
	# Draw the button bar
	set bn 0
	frame .bb.f1
	frame .bb.f2
	...
	foreach {name graph} $all_graphs {
		incr bn
		set b $side.b$bn
		button $b -text $name -command [list draw_graph $name $graph $arguments(-preview) $arguments(-gif)] -pady 0
		pack $b -side top -fill x -expand 1 -pady 0
		if {$side==".bb.f1"} {set side .bb.f2} {set side .bb.f1}
	}
	incr bn
	set b $side.b$bn
	button $b -text Everything -command {draw_all_graphs $arguments(-html)}
	...
	# silent mode
	# generate all graph or a specific one but with no window
	if { "all"==$arguments(-graph) } {
		puts "Generating all graphs..." 
		draw_all_graphs $arguments(-html)
	} else {
		set found 0
		foreach {name graph} $all_graphs {
			if { $name==$arguments(-graph) } {
				set found 1
				puts "Generating graph : $name ..." 
				draw_graph $name $graph $arguments(-preview) $arguments(-gif)
			}
		}
		if { 0 == $found } {
			puts "ERROR: Undefined graph $$arguments(-graph)"
		}
	}
	exit
}
